...
//            }
//
//            // 모든 미디어를 포함한 UserMessage 생성
//            UserMessage userMessage = new UserMessage(promptText, mediaList);
//
//            OpenAiChatOptions options = OpenAiChatOptions.builder()
//                    .withTemperature(0.7)
//                    .withModel("gpt-4o") // 멀티 이미지 분석에 최적화된 모델
//                    .build();
//
//            Prompt prompt = new Prompt(userMessage, options);
//            String responseContent = chatModel.call(prompt).getResult().getOutput().getContent();
...
//            return converter.convert(responseContent);
//        } catch (Exception e) {
//            log.error("AI 여러 이미지 분석 중 오류 발생", e);
//            throw new RuntimeException("AI 생성 실패", e);
//        }
//    }
...
//        String systemPromptText = new PromptTemplate(systemPromptResource).render();
//
//        // [수정] 여러 장의 사진을 분석하도록 지시문 강화
//        String multiImageInstruction = String.format(
//                "\n\n사용자가 총 %d장의 사진을 업로드했습니다. " +
//                        "모든 사진을 면밀히 검토하여 전체적인 맥락과 시간 흐름에 따른 일기를 작성하세요. " +
//                        "각 사진에 담긴 서로 다른 장소, 행동, 감정을 모두 반영해야 합니다.",
//                imageFiles.size()
//        );
//
//        String promptText = systemPromptText + multiImageInstruction + "\n\n" + converter.getFormat();
//
//        try {
//            List<Media> mediaList = new ArrayList<>();
//            for (MultipartFile file : imageFiles) {
//                // 로그로 파일이 정상적으로 읽히는지 확인
//                log.info("이미지 분석 준비 중: {} ({} bytes)", file.getOriginalFilename(), file.getSize());
//                mediaList.add(new Media(
//                        MimeTypeUtils.IMAGE_JPEG,
//                        new ByteArrayResource(file.getBytes())
//                ));
//            }
//
//            log.info("총 {}장의 이미지를 AI 모델(gpt-4o)에 전달합니다.", mediaList.size());
//
//            UserMessage userMessage = new UserMessage(promptText, mediaList);
//
//            OpenAiChatOptions options = OpenAiChatOptions.builder()
//                    .withTemperature(0.5) // 일관성을 위해 온도를 약간 낮춤
//                    .withModel("gpt-4o")
//                    .build();
//
//            Prompt prompt = new Prompt(userMessage, options);
